...
/*   Updated: 2023/07/09 22:46:18 by fgacougn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_vue_ligne(int *tab, int ligne, int *i4)
{
	...
	ft_init_tab(i, 3, 0);
	i[1] = tab[ligne + 2 * tab[0] + 1];
	i[4] = -1;
	while (i[0] < tab[0] && i[1] != -1)
	{
		if (ligne < tab[0])
			i[3] = ft_check_case(tab, ligne, i[0]);
		if (ligne >= tab[0])
			i[3] = ft_check_case(tab, ligne - tab[0], tab[0] - 1 - i[0]);
		if (i[3] > i[2])
		{
			i[2] = i[3];
			i[1]--;
...
return 0;
	...
	while (i[0] < tab[0] && i[1] != -1)
	{
		...
		if (colonne >= tab[0])
			i[3] = ft_check_case(tab, tab[0] - 1 - i[0], colonne - tab[0]);
		if (i[3] > i[2])
		{
			i[2] = i[3];
			i[1]--;
...
int	ft_check_plateau(int *tab)
{
	...
	i = 0;
	while (i < tab[0] * tab[0] && tab[i + 4 * tab[0] + 1] != 0)
		i++;
	soluce = i;
	i = 0;
	while (i < 4 * tab[0])
	{
		if (i < 2 * tab[0])
			...
			boo = ft_check_vue_ligne(tab, i - 2 * tab[0], &i4);
		if (boo < 0 || (boo == 0 && i4 == -1))
			return (-1);
		if (i++ != 100 && boo == 0)
			soluce++;
	}
	if (soluce == tab[0] * 4 + tab[0] * tab[0])
		return (1);
	return (0);
}
